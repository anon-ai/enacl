...
-define(CRYPTO_KDF_SALTBYTES, 16).
-define(CRYPTO_KDF_CONTEXTBYTES, 16).
...
%% Key Derivation
%% ---------------------
%% @doc kdf_keygen/1 creates a new master key of the given size.
%%
%% The KeyBytes can be between 16 and 64 bytes (128-512 bits).
%% @end
-spec kdf_keygen(KeyBytes) -> binary()
    when
      KeyBytes :: pos_integer().
kdf_keygen(KeyBytes) when KeyBytes >= ?CRYPTO_KDF_BYTES_MIN, KeyBytes =< ?CRYPTO_KDF_BYTES_MAX ->
    enacl_nif:crypto_kdf_keygen(KeyBytes);
kdf_keygen(_KeyBytes) ->
    error(badarg).

%% @doc kdf_derive_from_key/4 derives a subkey using the given subkey id, master key and context.
%%
%% The size of the key to generate, SubkeySize, should be between 16 and 64 bytes.
%%
%% SubkeyId should be a binary between 1 and 16 bytes long, and does not need to
%% be secret.
%%
%% The Context is another 1-16 byte binary that can be used to
%% guarantee that the same (subkey, master key) combination will produce
%% different outputs in different applications.
%%
%% The MasterKey should be generated by the kdf_keygen/1 function.
%% @end
-spec kdf_derive_from_key(SubkeyId, SubkeySize, Context, MasterKey) -> {ok, binary()} | no_return()
    when
      SubkeyId :: binary(),
      SubkeySize :: pos_integer(),
      Context :: binary(),
      MasterKey :: binary().
kdf_derive_from_key(SubkeyId, SubkeySize, Context, MasterKey) when
      is_binary(SubkeyId),
      byte_size(SubkeyId) =< ?CRYPTO_KDF_SALTBYTES,
      SubkeySize >= ?CRYPTO_KDF_BYTES_MIN,
      SubkeySize =< ?CRYPTO_KDF_BYTES_MAX,
      is_binary(Context),
      byte_size(Context) =< ?CRYPTO_KDF_CONTEXTBYTES ->
    enacl_nif:crypto_kdf_derive_from_key(SubkeyId, SubkeySize, Context, MasterKey);
kdf_derive_from_key(_SubkeyId, _SubkeySize, _Context, _MasterKey) ->
    error(badarg).
